...
bool SchedulerClient::RegisterWorker(const std::string& worker_id, const std::string& ip_address) {
...
bool SchedulerClient::SendHeartbeat(const std::string& worker_id, int running_task_count, float cpu_usage) {
    ...
    // 心跳失败通常不需要 Fatal，记个 Warn 即可，所以这里 CheckRpcStatus 内部打 Error 也没问题
...
bool SchedulerClient::UpdateTaskStatus(const std::string& task_id, 
    ...
    // 重要：确保 RPC 调用链追踪
    // 这里的 req_id 应该是当前线程正在处理的任务 ID (通过 thread_local t_req_id 获取)
    if (!dts::t_req_id.empty()) {
        context.AddMetadata("x-request-id", dts::t_req_id);
    }

    LOG_INFO << "Updating task " << task_id << " status to " 
         << dts::task::TaskState_Name(static_cast<dts::task::TaskState>(state));
