...
WorkerNode::WorkerNode(const std::string& worker_id, 
    ...
    auto channel = grpc::CreateChannel(scheduler_address, grpc::InsecureChannelCredentials());
...
void WorkerNode::OnTaskReceived(const dts::Task& task) {
    ...
    if (!handler) {
        ...
        // 注意：SchedulerClient 还需要适配，这里假设 SchedulerClient 已接受 dts::TaskState
        // 如果 SchedulerClient 还是用的 Proto Enum，我们需要强转，或者修改 Client 定义
        // 暂时假设 UpdateTaskStatus 接受的是你的 dts::TaskState
        scheduler_client_->UpdateTaskStatus(task.task_id, dts::TaskState::FAILED, 
    ...
    try {
        ...
        thread_pool_->enqueue([this, task, handler]() {
            ...
            try {
                // A. 执行业务逻辑
                // [修正] task.func_params 是 json 对象，需要 dump() 成 string 传给 handler
                // 假设 handler 签名是 string(string)
            ...
            scheduler_client_->UpdateTaskStatus(
                task.task_id, final_state, error_msg, result_json_str
            );
        });
    } catch (const std::exception& e) {
