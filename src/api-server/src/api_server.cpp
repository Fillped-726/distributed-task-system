...
/* 显式实例化（防止模板多次定义） */
template class AsyncCallContext<AsyncTaskService, PbSubmitDagRequest,
                                PbSubmitDagResponse>;
...
void AsyncServer::Run(uint16_t port) {
  if (!submit_task_) {
    LOG(FATAL) << "Server config error: 'submit_task_' handler is missing. "
                  "Server cannot start.";
  }
  ...
  for (int i = 0; i < c; ++i) {
    ...
    new AsyncCallContext<AsyncTaskService, PbSubmitDagRequest,
                         PbSubmitDagResponse>(
        &service_, cq, [this](auto* ctx) { OnSubmitDag(ctx); });
  }
